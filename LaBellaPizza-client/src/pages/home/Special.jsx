  ...
  return (
    <div className='section-container my-20 relative'>
      ...
      <div className='md:absolute right-3 top-8 mb-10 md:mr-24'>
        <button onClick={() => sliderRef.current.slickPrev()} className="btn p-2 rounded-full ml-5">
          <FaAngleLeft className='w-8 h-8 p-1'/>
        </button>
        <button onClick={() => sliderRef.current.slickNext()} className="btn p-2 rounded-full ml-5 bg-green">
        <FaAngleRight className='w-8 h-8 p-1'/>
        </button>
      </div>

      <Slider ref={sliderRef} {...settings} className='overflow-hidden mt-10 space-x-5'>
        {
          recipes.map((item, i) => (
