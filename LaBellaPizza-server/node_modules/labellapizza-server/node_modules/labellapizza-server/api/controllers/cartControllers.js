...
const addToCart = async(req, res) => {
    ...
    try{
        //exiting menu item
        const exitingCartItem = await Carts.findOne({menuItemId});
        if(exitingCartItem){
            return re.status(400).json({message: "Product already exists in the cart!"});
        }
...
const updateCart = async (req, res) => {
    ...
    try{
        ...
        if(!updateCart){
            return re.status(404).json({message: "Cart item is not found"})
        }
